        ...
        implements JoinableFilesystem
{
    ...
    public InfinispanFS( final String nodeKey, final Cache<String, FileMeta> metadataCache, final Cache<UUID, FileBlock> blockCache )
    {
    ...
    public JoinableFile getFile( final File file, final LocalLockOwner lockOwner, final StreamCallbacks callbacks,
                                 final boolean doOutput, final ReentrantOperationLock opLock )
            throws IOException
    {
        return new InfinispanJFS( file, lockOwner, callbacks, doOutput, opLock, this );
    }
    ...
    FileBlock getNextBlock( final FileBlock prevBlock )
    {
    ...
    void updateBlock( final FileBlock block )
            throws IOException
    {
        blockCache.put( block.getBlockID(), block );
    }

    void close( FileMeta metadata )
            throws IOException
    {
        try
        {
            lockManager.reentrantSynchronous( metadata.getFilePath(), ( opLock ) -> {
                ...
                // called from InfinispanJFS.reallyClose()? I think.
    ...
    FileMeta getMetadata( final File target, final LocalLockOwner owner )
            throws IOException
    {
